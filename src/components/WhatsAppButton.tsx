export default function WhatsAppButton({ whatsappNumber }: { whatsappNumber: string }) {
  ...
  return (
    <a
      ...
      className="group fixed bottom-5 right-5 z-50 flex h-14 w-14 items-center justify-center rounded-full bg-[#25D366] text-white shadow-lg shadow-black/20 transition-transform duration-300 hover:scale-110"
    >
      <span className="absolute inset-0 animate-ping rounded-full bg-[#25D366] opacity-60" />
      <svg viewBox="0 0 32 32" fill="currentColor" className="relative h-7 w-7 transition-transform duration-300 group-hover:rotate-12">
